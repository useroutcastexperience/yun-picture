...
public class DynamicShardingManager {
    ...
    private static final String DATABASE_NAME = "logic_db"; // ShardingSphere 默认逻辑数据库名称
    ...
    private void updateShardingTableNodes() {
        ...
        var databases = contextManager.getMetaDataContexts()
                .getMetaData()
                .getDatabases();
        log.info("ShardingSphere 中注册的数据库列表: {}", databases.keySet());

        var database = databases.get(DATABASE_NAME);
        if (database == null) {
            log.error("未找到数据库 '{}'，动态分表更新跳过。", DATABASE_NAME);
            return;
        }
